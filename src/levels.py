...
def _pivots(df: pd.DataFrame, k: int = 3):
    ...
    out = []
    for p, t in zip(hi_prices, df.index[piv_hi]):
        out.append({"time": t, "price": float(p), "kind": "pivot_high"})
    for p, t in zip(lo_prices, df.index[piv_lo]):
        out.append({"time": t, "price": float(p), "kind": "pivot_low"})
    return pd.DataFrame(out)


def _volume_profile_peaks(df: pd.DataFrame, bins: int = 200):
    # Approximate volume-at-price using hlc3 and volume
...
def _cluster_prices(prices: np.ndarray, tol: float):
    # Cluster by sorting and merging within tolerance
...
def build_levels_for_tf(df: pd.DataFrame, tf_name: str):
    ...
    # Tolerance scales with volatility: use median true range
    tr = (df["high"] - df["low"]).median()
    tol = float(tr * 0.75) if np.isfinite(tr) and tr > 0 else float(df["close"].median() * 0.001)

    prices = []
    if len(piv):
        prices += list(piv["price"].values)
    if len(vp):
        prices += list(vp["price"].values)
    ...
    # Rank: confluence strength proxy
...
def build_levels_multitf(tfs: dict):
    # Use higher TFs as the level sources (exclude 1m as it is too noisy)
    levels_by_tf = {}
    for tf in ["15m", "1h", "4h", "1d"]:
        if tf in tfs and len(tfs[tf]) > 50:
            levels_by_tf[tf] = build_levels_for_tf(tfs[tf], tf)
        else:
